...
const int mod=1e9+7;
int n,m;
ll f[maxn],invf[maxn],p[maxn][maxn];
ll qpow(ll a,ll b) {
	ll ans=1;
	while(b) {
...
void init() {
	...
	FOR(i,1,maxn) {
		p[i][0]=1;
		FOR(j,1,maxn) p[i][j]=p[i][j-1]*i%mod;
	}
}
...
int main() {
	...
	init();
	//For(i,0,3) printf("C(3,%d)=%lld\n",i,C(3,i));
	int T;scanf("%d",&T);
	while(T--) {
		scanf("%d%d",&n,&m);
		//if(m==1) { puts("0");continue; }
		ll ans=0;
		FOR(i,1,std::min(n+1,m)) ans+=((C(m,i)*C(n-1,n-i))%mod*f[i])%mod*C(std::max(n,m-i),n)%mod*p[std::min(m-i,n)][n]%mod;
		printf("%lld\n",ans%mod);
	}
